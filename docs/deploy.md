...
Beyond `docker compose`, the published image deploys to Azure Container Apps with the database and
evidence on an Azure Files share and the session secret and encryption key generated during the
deployment and kept in Key Vault, read by the app through a managed identity. There are two ways to
get there, producing the same resources.

**One click.** The **Deploy to Azure** button in
[`deploy/azure-arm/README.md`](../deploy/azure-arm/README.md) opens the Azure portal on an ARM
template: pick a subscription, a resource group and a region, optionally give the address of the
first administrator, and deploy. Nothing to install.

**Terraform**, for a deployment with a lifecycle — it plans changes before making them and keeps the
two secrets out of the deployment entirely, with ephemeral values and write-only arguments. The only
prerequisite is an `az login` session:
...
The walkthrough — including the first-administrator step and the caveats of running a single-writer
