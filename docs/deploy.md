...
Beyond `docker compose`, a Terraform example deploys the published image to Azure Container Apps,
with the database and evidence on an Azure Files share and the session secret and encryption key
generated by Terraform and kept in Key Vault. The only prerequisite is an `az login` session:
