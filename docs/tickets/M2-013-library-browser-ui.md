...
- [ ] With fixtures loaded in e2e/dev seed, user can search `T1059` and open the technique detail for
      the selected version.
- [ ] Switching ATT&CK version changes the technique detail identity (no stale cache across versions).
- [ ] Atomic procedure detail shows separate command and cleanup sections when both exist.
- [ ] Sigma detail shows reference-only messaging.
- [ ] Non-admin never sees enable/sync controls on this surface.
- [ ] Empty library CTA copy matches product decision (admin vs member).
