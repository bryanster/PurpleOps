...
def exportassessment(assessment, filetype):
    ...
    with open(
        f"{args.backupdir}/{str(assessment.id)}/export.csv",
        "w",
        encoding="UTF8",
        newline="",
    ) as f:
...
def exportentirebackupid(assessment):
    ...
    shutil.copytree(
        f"{args.files}/{str(assessment.id)}",
        f"{args.backupdir}/{str(assessment.id)}",
        dirs_exist_ok=True,
    )
    shutil.make_archive(
        f"{args.backupdir}/{str(assessment.id)}",
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument(
            "-d", "--dir", dest="backupdir", help="The backup directory", required=True
        )
        parser.add_argument(
            "-f",
            "--files",
            dest="files",
            help="The PurpleOps files directory",
            default="files",
        )
        parser.add_argument(
            "-H", "--host", help="Mongodb host. Default: host defined in flask.cfg"
        )
        parser.add_argument(
            "-p",
            "--port",
            type=int,
            help="Mongodb port. Default: port defined in flask.cfg",
        )
        ...
        if os.path.isdir(args.backupdir):
            if os.path.abspath(args.backupdir) == os.path.abspath(args.files):
                raise Exception(
                    "Invalid backup director, "
                    "can't backup to the PurpleOps files directory"
                )
