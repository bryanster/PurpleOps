...
from ...models.patch_engagement import PatchEngagement
...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Engagement | Problem | None:
    if response.status_code == 200:
        response_200 = Engagement.from_dict(response.json())
...
def sync_detailed(
    ...
    *,
    ...
    """Patch engagement fields.
     ...
     Lead and platform administrators. Every field is optional; only the ones
    present are changed. Changing `attack_version` after any step exists
    returns 409.

    Args:
        engagement_id (UUID):
        body (PatchEngagement): Every field is optional; only the ones present are changed.
