...
// Adapter is the CTID adversary-emulation plan catalog adapter.
//
...
type Adapter struct {
	...
	// bytes. Version is always storecontent.VersionCurrent.
	FetchBytes []byte
...
// New returns a production CTID adapter.
func New() *Adapter { return &Adapter{} }

// Kind implements [content.Adapter].
func (a *Adapter) Kind() storecontent.Kind { return storecontent.KindCTID }
...
func (a *Adapter) Fetch(ctx context.Context, req content.FetchRequest) (content.Bundle, error) {
	...
	if err != nil {
		return content.Bundle{}, fmt.Errorf("ctid: fetch %s: %w", url, err)
	}
	return content.Bundle{
		Bytes:     raw,
		Size:      int64(len(raw)),
...
func (a *Adapter) Parse(ctx context.Context, bundle content.Bundle) (content.AST, error) {
	...
	if len(raw) == 0 && bundle.Path != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("ctid: parse: read path: %w", err)
		}
	}
	doc, err := parseCTID(raw)
	if err != nil {
		return nil, err
	}
	return doc, nil
...
func (a *Adapter) Normalize(ctx context.Context, ast content.AST) ([]content.Object, error) {
	...
	doc, ok := ast.(*ctidDoc)
	if !ok {
		return nil, fmt.Errorf("ctid: normalize: unexpected AST type %T", ast)
	}
...
func (a *Adapter) Apply(ctx context.Context, w content.Writer, objects []content.Object, prog content.Progress) error {
	if len(objects) != 1 {
		return fmt.Errorf("ctid: apply: expected 1 catalog object, got %d", len(objects))
	}
	cat, ok := objects[0].(*Catalog)
	if !ok {
		return fmt.Errorf("ctid: apply: unexpected object type %T", objects[0])
	}
