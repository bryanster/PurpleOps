...
// Adapter is the SigmaHQ content adapter.
//
...
// New returns a production Sigma adapter.
func New() *Adapter { return &Adapter{} }

// Kind implements [content.Adapter].
func (a *Adapter) Kind() storecontent.Kind { return storecontent.KindSigma }
...
func (a *Adapter) Fetch(ctx context.Context, req content.FetchRequest) (content.Bundle, error) {
	...
	if url == "" {
		return content.Bundle{}, fmt.Errorf("sigma: fetch: source URL is empty")
	}
...
func (a *Adapter) Parse(ctx context.Context, bundle content.Bundle) (content.AST, error) {
	...
	if len(raw) == 0 && bundle.Path != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("sigma: parse: read path: %w", err)
		}
	}
	doc, err := parseSigma(raw)
	if err != nil {
		return nil, err
	}
	return doc, nil
...
func (a *Adapter) Normalize(ctx context.Context, ast content.AST) ([]content.Object, error) {
	...
	doc, ok := ast.(*sigmaDoc)
	if !ok {
		return nil, fmt.Errorf("sigma: normalize: unexpected AST type %T", ast)
	}
...
func (a *Adapter) Apply(ctx context.Context, w content.Writer, objects []content.Object, prog content.Progress) error {
	if len(objects) != 1 {
		return fmt.Errorf("sigma: apply: expected 1 catalog object, got %d", len(objects))
	}
	cat, ok := objects[0].(*Catalog)
	if !ok {
		return fmt.Errorf("sigma: apply: unexpected object type %T", objects[0])
	}
