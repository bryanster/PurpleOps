...
func TestCreateZip(t *testing.T) {
	...
	srcDir := t.TempDir()
	os.WriteFile(filepath.Join(srcDir, "file1.txt"), []byte("hello"), FilePerm)
	os.MkdirAll(filepath.Join(srcDir, "subdir"), DirPerm)
	os.WriteFile(filepath.Join(srcDir, "subdir", "file2.txt"), []byte("world"), FilePerm)
...
func TestCopyDir(t *testing.T) {
	srcDir := t.TempDir()
	os.WriteFile(filepath.Join(srcDir, "a.txt"), []byte("alpha"), FilePerm)
	os.MkdirAll(filepath.Join(srcDir, "sub"), DirPerm)
	os.WriteFile(filepath.Join(srcDir, "sub", "b.txt"), []byte("beta"), FilePerm)

	dstDir := filepath.Join(t.TempDir(), "copy")
	os.MkdirAll(dstDir, DirPerm)
