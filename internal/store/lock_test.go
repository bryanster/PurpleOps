...
var errLockConflict = fmt.Errorf("database/sql/driver: could not connect to database: %w",
	&duckdb.Error{
		...
		Msg: `IO Error: Could not set lock on file "/var/lib/purpleops/purpleops.duckdb": ` +
			`Conflicting lock is held in /usr/local/bin/purpleops (PID 1) by user purpleops. ` +
...
func TestALockConflictIsTaggedAsSuch(t *testing.T) {
	...
	err := openError("/var/lib/purpleops/purpleops.duckdb", errLockConflict)
	...
	for _, want := range []string{"/var/lib/purpleops/purpleops.duckdb", "PID 1"} {
...
func TestOtherFailuresAreNotTaggedAsLocked(t *testing.T) {
	...
	tests := map[string]error{
		"another IO error": fmt.Errorf("connect: %w", &duckdb.Error{
			Type: duckdb.ErrorTypeIO,
			Msg:  `IO Error: Cannot open file "/var/lib/purpleops/purpleops.duckdb": Permission denied`,
		}),
