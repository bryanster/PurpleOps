...
def pullMitreAttack(component):
    ...
    req = requests.get(
        f"https://github.com/CyberCX-STA/PurpleOps-Deps/raw/master/attack.mitre/15.1/enterprise-attack-v15.1-{component}.xlsx"
    )
    if req.status_code == 200:
        # req = r.text.split('"')
        # url = [
        #     x for x in req
        #     if "xlsx" in x and "enterprise" in x and "docs" in x and component in x
        # ][0]
...
def pullSigma():
    if os.path.exists(f"{PWD}/external/sigma") and os.path.isdir(
        f"{PWD}/external/sigma"
    ):
...
def parseSigma():
    ...
    for sigmaRule in glob(f"{PWD}/external/sigma/rules/**/*.yml", recursive=True):
        ...
        for ttp in associatedTTP:
            Sigma(
                mitreid=ttp, name=yml["title"], description=yml["description"], url=url
            ).save()
...
def pullAtomicRedTeam():
    ...
    Repo.clone_from(
        "https://github.com/redcanaryco/atomic-red-team", f"{PWD}/external/art"
    )
    ...
    for artTestcases in glob(f"{PWD}/external/art/atomics/T*/*.yaml", recursive=True):
        ...
        for artTestcase in yml["atomic_tests"]:
            ...
            if "command" in artTestcase["executor"]:
                ...
                if "input_arguments" in artTestcase and isinstance(
                    artTestcase["input_arguments"], dict
                ):
                    for i in artTestcase["input_arguments"].keys():
                        k = "#{" + i + "}"
                        baseCommand = baseCommand.replace(
                            k, str(artTestcase["input_arguments"][i]["default"])
                        )
...
def populateSecrets():
    if Role.objects().count() == 0:
        ...
        dotenv.set_key(dotenvFile, "FLASK_SECRET_KEY", secrets.token_urlsafe())
        dotenv.set_key(
            dotenvFile,
...
if Tactic.objects.count() == 0:
    print("==============================================================\n\n\n")
    print("\t NEW INSTANCE DETECTED, LETS GET THE DATA WE NEED")
    print("\n\n\n==============================================================")
